  ...
  if (req.method === 'POST') {
    try {
      ...
      const newSong: Song = {
        ...
        coverUrl: (typeof coverUrl === 'string' && coverUrl.trim()) ? coverUrl.trim().slice(0, 500) : 'https://images.unsplash.com/photo-1511671782779-c97d3d27a1d4?auto=format&fit=crop&w=800&q=80',
        category: getValidCategory(category),
        spotifyUrl: (typeof spotifyUrl === 'string') ? spotifyUrl.trim().slice(0, 500) : '',
        youtubeUrl: (typeof youtubeUrl === 'string') ? youtubeUrl.trim().slice(0, 500) : '',
        audioUrl: (typeof audioUrl === 'string' && audioUrl.trim()) ? audioUrl.trim().slice(0, 500) : 'https://cdn.pixabay.com/download/audio/2022/05/27/audio_1808fbf07a.mp3?filename=lofi-chill-112194.mp3',
